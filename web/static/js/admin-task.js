function getIdsFromUrl() {
    ...
    const pathSegments = url.pathname.split('/'); // Разбиваем путь на сегменты

    // Предполагаем, что структура URL: /admin/projects/{projectId}/tasks/{taskId}
    const projectId = pathSegments[pathSegments.indexOf('projects') + 1];
    const taskId = pathSegments[pathSegments.indexOf('tasks') + 1];

    return {
        projectId: parseInt(projectId, 10), // Преобразуем в число
        taskId: parseInt(taskId, 10)          // Преобразуем в число
    };
}
...
function updateBreadcrumb(projectId, projectTitle, taskTitle) {
    ...
    breadcrumb.innerHTML = `
        ...
        <li><a href="/admin/projects/${projectId}">${projectTitle}</a></li>
...
async function deleteTask() {
    ...
    if (confirm("Вы уверены, что хотите удалить эту задачу?")) {
        try {
            ...
            // Обновление страницы или перенаправление, если необходимо
            window.location.href = `http://localhost:8080/admin/projects/${projectId}/`;
        } catch (error) {
